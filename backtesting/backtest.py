...
def manage_trade(position, row, balance, daily_loss, total_profit, total_loss, profit_trades, loss_trades):
    if row['low'] <= position['stop_loss']:  # Stop-loss hit
        ...
        return None, balance, daily_loss, total_profit, total_loss, profit_trades, loss_trades
    ...
    return position, balance, daily_loss, total_profit, total_loss, profit_trades, loss_trades

# Function to run the backtest with an optional candle limit
def run_backtest(instrument, data, atr_multiplier=1.5, stop_loss_atr=1.0, candle_limit=None):
    trades = []
    position = None
    balance = 10000
    daily_loss = 0
    ...
    for i in range(200, candle_count):  # Start after 200 rows for MA calculations
        ...
        # Manage open position
            ...
            position, balance, daily_loss, total_profit, total_loss, profit_trades, loss_trades = manage_trade(
...
def backtest_instruments(input_dir, output_base_dir, pair=None):
    ...
    if pair:
        ...
        if os.path.exists(filepath):
            ...
            trades = run_backtest(pair, df, candle_limit=CANDLE_LIMIT)
    ...
    for file in os.listdir(input_dir):
        if file.endswith(".csv"):
            ...
            # Run backtest
            trades = run_backtest(instrument, df, candle_limit=CANDLE_LIMIT)
